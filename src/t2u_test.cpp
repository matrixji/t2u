...
void log(int level, const char *mess)
{
    if (level >= 0)
    {
...
int main(int argc, char *argv[])
{
    ...
    context_s=init_forward(sock_s);
    rule_s[0] = add_forward_rule(context_s, forward_server_mode, "test_http", 80);
    rule_s[1] = add_forward_rule(context_s, forward_server_mode, "test_ssh", 22);


    context_c=init_forward(sock_c);
