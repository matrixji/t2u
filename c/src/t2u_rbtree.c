...
static inline int compare_default(void* key_a,void* key_b)
{
    if(key_a > key_b)
        return 1;
    else if(key_a == key_b)
...
struct rbtree* rbtree_init(rbtree_cmp_proc compare)
{
    ...
    if (NULL == tree->compare)
    {
        tree->compare = compare_default;
    }
